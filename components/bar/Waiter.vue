...
const coupons = computed(() => lines.value.reduce((a: number, l: any) => a + l.couponQty, 0))
    ...
    <template v-if="tab === 'new'">
      ...
      <template v-for="[cat, items] in menu" :key="cat">
        ...
        <div v-for="m in items" :key="m.id" class="item" :class="{ cp: coupon[m.id] }">
          <div class="nm">{{ m.name }}<span v-if="m.couponOk && qty[m.id]" class="cpn" :class="{ on: coupon[m.id] }" @click.stop="bumpCoupon(m.id)">🎟 {{ coupon[m.id] ? `${coupon[m.id]} κουπόνι` : 'κουπόνι;' }}</span></div>
