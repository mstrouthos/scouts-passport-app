...
onMounted(() => { loadBalances(); balTimer = setInterval(loadBalances, 5000) })
...
<template>
  ...
  <div v-for="r in tables" :key="r.no" class="order" @click="open = open === r.no ? null : r.no">
    <div class="hd"><span class="tb">Τραπέζι {{ r.no }}</span>
      <span class="meta"><template v-if="balances[r.no]?.issued">🎟 {{ balances[r.no].used }}/{{ balances[r.no].issued }} · </template><template v-if="people && people[r.no]">👥 {{ people[r.no].arrived }}/{{ people[r.no].booked }} · </template>{{ r.orders.length }} {{ r.orders.length === 1 ? 'παραγγελία' : 'παραγγελίες' }}<br><span v-if="r.owed" class="pill unpaid">οφείλει {{ eur(r.owed) }}</span><span v-else class="pill paid">εξοφλημένο</span></span></div>
