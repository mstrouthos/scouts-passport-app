...
/** A parent is in scope when any of their children's sections is. Scoping on
    the children means a scout moving up carries their parents' visibility with
    them, and a family with a Λυκόπουλο and a Πρόσκοπος answers to both. */
export async function parentInScope(me: any, id: number) {
  ...
  if (secIds !== null) {
    const secs = sectionsOfParent(p,
      await db.select().from(s.parentChildren),
      await db.select().from(s.scouts),
      await db.select().from(s.patrols))
    if (!secs.some(sec => secIds.includes(sec)))
