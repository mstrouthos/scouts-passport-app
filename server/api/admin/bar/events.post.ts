...
export default defineEventHandler(async (event) => {
  ...
  const b = await readBody<{ name?: string; eventDate?: string; tableCount?: number; copyMenuFrom?: number }>(event)
  ...
  if (b?.copyMenuFrom) {
    ...
    if (src.length) await db.insert(s.barMenuItems).values(src.map(m => ({
      eventId: row.id, category: m.category, name: m.name, priceCents: m.priceCents, sort: m.sort, isActive: true
    })))
  }
  return { id: row.id }
})
